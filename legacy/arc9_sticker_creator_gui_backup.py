...
class StickerCreatorGUI(tk.Tk):
    ...
    def start_processing(self, image_folder, pack_name, output_dir):
        self.output_dir = output_dir # Set the output directory for this run
        images_to_process = []
...
class SetupFrame(tk.Frame):
    def __init__(self, parent, controller):
        ...
        tk.Label(frame, text="Output Folder", bg=controller.BG_COLOR, fg=controller.FG_COLOR, font=controller.FONT_NORMAL).grid(row=1, column=0, padx=15, pady=10, sticky="w")
        
        self.output_folder_path_var = tk.StringVar(value=self.controller.output_dir)
        output_folder_entry = tk.Entry(frame, textvariable=self.output_folder_path_var, 
                                bg=controller.ENTRY_BG, fg=controller.FG_COLOR,
                                relief='flat', state='readonly', readonlybackground=controller.ENTRY_BG,
                                font=controller.FONT_NORMAL)
        output_folder_entry.grid(row=1, column=1, padx=5, pady=10, sticky="ew", ipady=4)

        output_browse_button = tk.Button(frame, text="Browse...", 
                                  bg=controller.BTN_SECONDARY_BG, fg='white', 
                                  activebackground='#777777', activeforeground='white',
                                  command=self.browse_output_folder, relief='flat', borderwidth=0,
                                  font=controller.FONT_BOLD)
        output_browse_button.grid(row=1, column=2, padx=(5,15), pady=10, ipady=2, ipadx=5)

        tk.Label(frame, text="Pack Name", bg=controller.BG_COLOR, fg=controller.FG_COLOR, font=controller.FONT_NORMAL).grid(row=2, column=0, padx=15, pady=10, sticky="w")
        ...
        self.pack_name_entry = tk.Entry(frame, textvariable=self.pack_name_var,
                                   ...
                                   font=controller.FONT_NORMAL)
        self.pack_name_entry.grid(row=2, column=1, columnspan=1, padx=5, pady=10, sticky="ew", ipady=4)
    ...
    def browse_output_folder(self):
        folder_selected = filedialog.askdirectory(initialdir=self.controller.output_dir)
        if folder_selected:
    ...
    def validate(self):
        ...
        if not pack_name_ok:
            self.caution_label.grid(row=2, column=2, padx=(0, 15), sticky='w')
    ...
    def on_start(self):
        ...
        if self.validate():
            ...
            output_dir = self.output_folder_path_var.get()
            self.controller.start_processing(self.folder_path_var.get(), pack_name, output_dir)
