...
def create_addon_structure(pack_name):
    """Create the necessary directory structure for the ARC9 addon."""
    os.makedirs(f"arc9_{pack_name}_stickers/lua/arc9/common/attachments_bulk", exist_ok=True)
    os.makedirs(f"arc9_{pack_name}_stickers/materials/stickers/{pack_name}", exist_ok=True)

def create_vtf_and_vmt(image_obj, pack_name, compact_name):
    """Create a static DXT5 VTF and VMT file from a given PIL Image object."""
    vtf_path = f"arc9_{pack_name}_stickers/materials/stickers/{pack_name}/{compact_name}.vtf"
    vmt_path = f"arc9_{pack_name}_stickers/materials/stickers/{pack_name}/{compact_name}.vmt"
    vtf_lib = None
    try:
...
def create_lua_script(pack_name, processed_images):
    """Create the Lua script for the ARC9 addon from pre-processed info."""
    lua_path = f"arc9_{pack_name}_stickers/lua/arc9/common/attachments_bulk/a9sm_{pack_name}.lua"
...
def create_addon_json(pack_name):
    """Create the addon.json file."""
    json_path = f"arc9_{pack_name}_stickers/addon.json"
    with open(json_path, "w") as f:
        f.write(f'''{{
  "title": "[ARC9] {pack_name.replace('_', ' ').title()} - Stickers",
  "type": "weapon",
  "tags": [
    "fun"
  ],
...
def main():
    ...
    image_folder = input("Enter the name of the folder with images: ")
    ...
    create_addon_structure(pack_name)
    successful_images = []
    ...
    for info in processed_info:
        ...
        try:
            ...
            if image_obj and create_vtf_and_vmt(image_obj, pack_name, info["compact_name"]):
    ...
    if successful_images:
        ...
        create_lua_script(pack_name, successful_images)
        create_addon_json(pack_name)
