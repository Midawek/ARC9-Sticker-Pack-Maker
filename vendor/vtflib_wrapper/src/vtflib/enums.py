...
class EnumerationType(type(c_uint32)):
    def __new__(metacls, name, bases, dict_):
        if "_members_" not in dict_:
            _members_ = {}
            for key, value in dict_.items():
                if not key.startswith("_"):
                    _members_[key] = value

            dict_["_members_"] = _members_
        else:
            _members_ = dict_["_members_"]

        dict_["_reverse_map_"] = {v: k for k, v in _members_.items()}
        cls = type(c_uint32).__new__(metacls, name, bases, dict_)
...
class HeightConversionMethod(CEnumeration):
    ...
    HeightConversionMethodGreen = 4
    HeightConversionMethodBlue = 5
    HeightConversionMethodMaxRGB = 6
    HeightConversionMethodColorSpace = 7
