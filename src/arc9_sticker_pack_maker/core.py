...
if getattr(sys, 'frozen', False):
    ...
    OUTPUT_ADDON_ROOT = Path(sys.executable).parent
else:
    BASE_PATH = Path(__file__).resolve().parents[2]
    OUTPUT_ADDON_ROOT = BASE_PATH

# --- Consolidate sys.path modification here ---
LIBS_PATH = BASE_PATH / "vendor" / "libs"
VTFLIB_PATH = BASE_PATH / "vendor" / "vtflib_wrapper" / "src"
for dependency_path in (LIBS_PATH, VTFLIB_PATH):
    dependency_path_str = str(dependency_path)
    if dependency_path_str not in sys.path:
        sys.path.insert(0, dependency_path_str)

# Add the project base path for local modules and resources.
base_path_str = str(BASE_PATH)
if base_path_str not in sys.path:
    sys.path.insert(1, base_path_str)
...
try:
    from vtflib import VTFLib, ImageFlag, ImageFormat
except ImportError as e:
    print("ERROR: Could not import the VTFLib wrapper.")
    print("Please ensure the local 'vendor/vtflib_wrapper' package is present.")
    if sys.stdin and sys.stdin.isatty():
        input("\nPress Enter to exit.")
    sys.exit(1)
...
def sanitize_for_filename(name, strict=True):
    """Removes emojis. If strict, converts to lowercase and removes spaces/special chars. If not strict, preserves case and allows spaces."""
    name_no_emoji = remove_emojis(name)
    if strict:
        # Replace spaces with underscores before stripping other characters
        name_clean = name_no_emoji.replace(' ', '')
...
def iter_sorted_files(folder_path):
    ...
    yield from sorted(files, key=lambda entry: entry.name.lower())

def discover_images(folder_path):
    """Return Pillow-readable image metadata from a folder."""
    images = []
    for entry in iter_sorted_files(folder_path):
        try:
...
def process_image_to_vtf(output_path, image_info, pack_name, compact_name, sticker_dir):
    ...
    try:
        with Image.open(image_info["path"]) as img:
            ...
            options.ImageFormat = ImageFormat.ImageFormatDXT5
            options.Flags |= ImageFlag.ImageFlagEightBitAlpha
            ...
            if is_animated:
                ...
                if not vtf_lib.image_create(w, h, len(frames), 1, 1, options.ImageFormat.value, False, False, True):
...
┗━ ╹ ╹╹┗╸┗━╸┗━┛ ━┛   ┗━┛ ╹ ╹┗━╸╹ ╹┗━╸╹┗╸   ╹  ╹ ╹┗━╸╹ ╹   ╹ ╹╹ ╹╹ ╹┗━╸╹┗╸ ╹  ╹ 
    ...
    if getattr(sys, 'frozen', False):
        output_path = str(OUTPUT_ADDON_ROOT)
    else:
        output_path = str(OUTPUT_ADDON_ROOT)
    ...
    print("\nScanning for images...")
    images_to_process = discover_images(image_folder_path)
    for image_info in images_to_process:
        print(
            f"Found {image_info['type']} image: "
            f"{os.path.basename(image_info['path'])}"
        )
    ...
    for item in images_to_process:
        ...
        compact_name = sanitize_for_filename(print_name, strict=True)
